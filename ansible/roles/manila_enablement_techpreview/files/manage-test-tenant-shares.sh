...
#     --external-network <name>    External network for router gateway and
#                                  floating IPs (default: $EXTERNAL_NETWORK or PUBLICNET)
#
...
if [[ "$ACTION" != "create" && "$ACTION" != "destroy" ]]; then
  ...
  echo "  --external-network <name>    External network for router gateway and"
  echo "                               floating IPs (default: \$EXTERNAL_NETWORK or PUBLICNET)"
  exit 1
fi
